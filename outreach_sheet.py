...
from outreach_parser import main as parse_main
...
def _scraper(request_url: str, which_: str = "organization"):
    ...
    response = request_method(method, url, payload, headers, isJson=False)
    if response.status_code != 200:
        raise ValueError(f"Scraper stopped working with status code: {response.status_code}. Message: {response.text}")
...
def parser(df):
    ...
    df = df.dropna(subset=['websiteUrl'])
...
def main(file_path: str, start_from: str = ""):
    ...
    for _, row in df.iterrows():
        print(row['websiteUrl'])
        url = _parse_url(row['websiteUrl'])
...
if __name__ == "__main__":
    start_from = ""
    main(read_file, start_from)
    print("-" * 100)
    parse_main(save_path)
